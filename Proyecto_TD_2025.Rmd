...
procesar_ticket <- function(fichero) {
  ...
  for (linea in lineas) {
    ...
    if (ticket_valido) {
      ...
      if (en_productos && !str_detect(linea, "TOTAL [(]€[)]")) {
        ...
        # Detecta la línea típica de producto con cantidad, descripción, precio unitario e importe total.
        if (str_detect(linea, "^[[:digit:]]+[ ]+.+[ ]+[[:digit:]]+,[[:digit:]]{2}[ ]+[[:digit:]]+,[[:digit:]]{2}$")) {
          partes_p <- strsplit(linea, "[ ]+")[[1]]
          cantidad <- as.numeric(partes_p[1])
          if (length(partes_p) == 4) {
            # Hay peso
            descripcion       <- partes_p[2]
            peso              <- partes_p[3]
            precio_unitario   <- partes_p[4]
          } else if (length(partes_p) == 3) {
            # No hay peso
            descripcion       <- partes_p[2]
            peso              <- NA
            precio_unitario   <- partes_p[3]
          } else {
            # Caso inesperado
            descripcion     <- NA
            peso            <- NA
        ...
        if (str_detect(str_trim(linea), "^[[:digit:]]+ kg[ ]+[[:digit:]]€/kg[ ]+[[:digit:]]+$")) {
          partes_p1 <- strsplit(linea, "[ ]+")[[1]]
          cantidad <- as.numeric(partes_p1[1])
          descripcion <- partes_p1[2]
          
        } 
        
        if (str_detect(str_trim(linea), "^[[:digit:]]+[ ]+[[:alpha:]]+$")) {
          partes_p2 <- strsplit(linea, "[ ]+")[[1]]
          Peso <- partes_p2[1]
          P.Unit <- partes_p2[2]
          Importe <- partes_p2[3]
        }
  ...
  # Construimos el data frame global del ticket con los datos extraídos.
  ticket_df <- data.frame(
    ...
    precio_unitario = I(list(precio_unitario)),
    importe = I(list(importe)),
  ...
  # Retornamos ambos data frames en una lista.
