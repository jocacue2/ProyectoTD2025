...
procesar_ticket <- function(fichero) {
  ...
  for (linea in lineas) {
    ...
    if (ticket_valido) {
      ...
      if (en_productos && !str_detect(linea, "TOTAL \\(€\\)")) {
        ...
        # Linea que detecte pescado.
        else if (str_detect(linea, "^PESCADO$") | zona_pesos == TRUE %% !str_detect(linea, "TOTAL \\(€\\)")) {
                zona_pesos <- TRUE  
          ...
          if (str_detect(linea, "^[A-ZÁÉÍÓÚÜÑ]+(?:\s+[A-ZÁÉÍÓÚÜÑ]+)*$")) {
                   descripcion <- linea
                    ...
                    cantidad <- matches[1, 2]  # Extrae la cantidad
                    descripcion <- matches[1, 3]}}
  
          else if (str_detect(linea, "^([0-9]+,[0-9]+) +kg +([0-9]+,[0-9]+) €/kg +([0-9]+,[0-9]+)$")) {
                  matches <- str_match(linea, "^([0-9]+,[0-9]+) +kg +([0-9]+,[0-9]+) €/kg +([0-9]+,[0-9]+)$")
                  peso <- str_replace_all(matches[2], " kg", "")  # Eliminamos "kg"
                  precio_por_kg <- str_replace_all(matches[3], " €/kg", "")  # Eliminamos "€/kg"
                  importe <- matches[4]
      ...
      if (en_total) {
        ...
        # Detección de los tipos de IVA (porcentaje, base y cuota).
  ...
  # Unificamos las cadenas de IVA detectadas.
