...
procesar_ticket <- function(fichero) {
  ...
  AUT <- NA
  ARC <- NA
  productos <- data.frame(Descripcion = character(), P.Unit = character(), Importe = character(), Peso = character(), factura_simplificada = character())
  ...
  for (linea in lineas) {
    ...
    if (ticket_valido) {
      ...
      #Extraemos el número de la factura simplificada de la sexta línea mediante la división de la misma por los dos puntos.
  ...
  linea <- str_trim(lineas[i]) 
     ...
     # Si ya hemos recorrido las líneas posteriores al TOTAL, salimos del bucle principal.
    break
  }
